...
#Paquete para gráfico de velocímetros
from streamlit_echarts import st_echarts

#1 CONFIGURACION DE LA PÁGINA
st.set_page_config(
     page_title = 'Risk Score Analyzer',
     page_icon = 'banco_santafe.png',
     layout = 'wide') #Formato ancho
...
with st.sidebar:
    ...
    principal = st.number_input('Importe Solicitado', 500, 50000) # Inputs máximo y minimo que puede poner el usuario.
...
if st.sidebar.button('CALCULAR RIESGO'):
    ...
    kpi_lg = int(EL.lg * 100)
    kpi_el = int(EL.principal * EL.pd * EL.ead * EL.lg)
    ...
    with col1:
        st_echarts(options=pd_options, width="110%", key=0)# Identificador, no tiene que tener un orden especial, pero sí ser diferentes.
    with col2:
